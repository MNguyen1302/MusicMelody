// AUTH
export const AUTH_SUCCESS = 'AUTH_SUCCESS';
export const AUTH_GOOGLE_SUCCESS = 'AUTH_GOOGLE_SUCCESS';
export const AUTH_FAIL = 'AUTH_FAIL';
export const REGISTER = 'REGISTER';
// USER
export const GET_USER = 'GET_USER';
export const EDIT_USER = 'EDIT_USER';
export const CHANGE_PASSWORD = 'CHANGE_PASSWORD';
export const GET_FAVOURITE = 'GET_FAVOURITES';
export const USER_LOGOUT = 'LOGOUT';
// SONG
export const ADD_CURRENT_SONG = 'ADD_CURRENT_SONG';
export const PLAY_PAUSE_SONG = 'PLAY_PAUSE_SONG';
export const GET_ALL_SONGS = 'GET_ALL_SONGS';
export const GET_TOP_TRENDING = 'GET_TOP_TRENDING';
export const GET_SONG = 'GET_SONG';
export const GET_CATEGORY = 'GET_CATEGORY';
export const LIKE_SONG = 'LIKE_SONG';
// ARTIST
export const GET_ALL_ARTIST = 'GET_ALL_ARTIST';
export const GET_ARTIST = 'GET_ARTIST';
export const FOLLOW_ARTIST = 'FOLLOW_ARTIST';
// COMMENT
export const POST_COMMENT = 'POST_COMMENT';
// ADMIN
export const CREATE_SONG = 'CREATE_SONG';
export const CREATE_ARTIST = 'CREATE_ARTIST';
export const EDIT_SONG = 'EDIT_SONG';
export const DELETE_SONG = 'DELETE_SONG';
// LOADING
export const TOGGLE_LOADING = 'TOGGLE_LOADING';
// PLAYLIST
export const CREATE_PLAYLIST = 'CREATE_PLAYLIST';
export const GET_PLAYLIST = 'GET_PLAYLIST';
export const GET_PLAYLIST_DETAIL = 'GET_PLAYLIST_DETAIL';
export const UPDATE_PLAYLIST = 'UPDATE_PLAYLIST';
export const ADD_CURRENT_PLAYLIST = 'ADD_CURRENT_PLAYLIST';
export const PLAY_PAUSE_PLAYLIST = 'PLAY_PAUSE_PLAYLIST';
export const GET_TRACK_INDEX = 'GET_TRACK_INDEX';
export const GET_CURRSONG_IN_PLAYLIST = 'GET_CURRSONG_IN_PLAYLIST';